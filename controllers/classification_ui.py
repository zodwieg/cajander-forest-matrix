# controllers/classification_ui.py
...
class ClassificationUIController:
    ...
    def __init__(self, main_dialog, iface, execute_algorithm_callback):
        ...
        self.iface = iface
        # Запоминаем коллбэк для вызова тяжелого алгоритма (который скинешь следующим сообщением)
        self.execute_algorithm_callback = execute_algorithm_callback

        # Подписываемся на сигнал запуска расчета с первой вкладки через главное окно
    ...
    def handle_param_toggle(self, biome_id: str, param_id: str, is_enabled: bool):
        ...
        # Записываем состояние в модель, обновляем синглтон и сохраняем JSON
    ...
    def _on_run_requested(self, state_dict: dict, should_replace: bool):
        ...
        # Делегируем выполнение тяжелого метода обратно главному контроллеру
        # (Сам метод _execute_algorithm мы разберем в следующем шаге)
